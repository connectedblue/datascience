...
analytic_summary <- as.data.frame(sapply(analytic_summary, format, digits=0, big.mark = ',', scientific=999, trim=TRUE   ))
...
summary_table_twocol<- function (df,grp_by, col_1, col_2, caption="", top=10){
        ...
        top_impacts <- cbind(data.frame(n=c(1:top,"","")),
                                 ...
                                 totalcrunch(rhs , 1:top, "(OTHER)","&nbsp;")
                                 )
        top_impacts <- sapply(top_impacts, format, digits=0, big.mark = ',', scientific=999 )
...
summary_table_fourcol<- function (df,grp_by, col_1, col_2, col_3, col_4,caption="",
        ...
        top_impacts <- cbind(data.frame(n=c(1:top,"","")),
                                 ...
                                 totalcrunch(c2 , 1:top, "(OTHER)","&nbsp;"),
                                 totalcrunch(c3 , 1:top, "(OTHER)","&nbsp;"),
                                 totalcrunch(c4 , 1:top, "(OTHER)","&nbsp;")
                                 )
        ...
        kable(top_impacts, digits=0, format.args=list(big.mark = ','),
              caption = caption,
              col.names = c("","", col_1[2], "", col_2[2], "", col_3[2], "", col_4[2]))
...
summary_table_twocol(analysis_annual_events, "EVTYPE", c("event_fatalities","Fatalities"), c("event_injuries","Injuries"), sprintf("Top 10 Public Health impacts %d - 2011", start_year))
...
summary_table_twocol(analysis_annual_events, "EVTYPE", c("event_property_damage","Property Damage (&dollar;’000)"), c("event_crop_damage","Crop Damage (&dollar;’000)"), sprintf("Top 10 Economic impacts %d - 2011", start_year))
...
```{r resultsstates}
summary_table_fourcol(analysis_annual_states, "STATE", c("event_fatalities","Fatalities"), c("event_injuries","Injuries"), c("event_property_damage","Property Damage (&dollar;’000)"), c("event_crop_damage","Crop Damage (&dollar;’000)"), sprintf("Top 10 impacted states %d - 2011", start_year))
