...
---------------------------     -----------------------------------------------
Number of obsersavations        `r sum(annual_events$total_annual_events)`
Number of Fatalities            `r sum(annual_events$total_annual_fatalities)`
Number of obsersavations        `r sum(annual_events$total_annualinjuries)`
...
# a grand total row
totalcrunch <- function(df, rowstokeep, rolleduptext, totaltext="GRAND TOTAL"){
...
summary_table<- function (df,grp_by, col_1, col_2, caption="", top=10){
        lhs <- sumcolumn(df, grp_by,col_1[1])
        rhs <- sumcolumn(df, grp_by,col_2[1])
        top_impacts <- cbind(data.frame(n=c(1:top,"","")),
                                 totalcrunch(lhs, 1:top, "(OTHERS)"),
                                 totalcrunch(rhs , 1:top, "(OTHERS)","")
                                 )
        
        kable(top_impacts, digits=0, format.args=list(big.mark = ','),
              caption = caption,
              col.names = c("","", col_1[2], "", col_2[2]))
...
summary_table(analysis_annual_events, "EVTYPE", c("event_fatalities","Fatalities"), c("event_injuries","Injuries"), sprintf("Top 10 Public Health impacts %d - 2011", start_year))
...
summary_table(analysis_annual_events, "EVTYPE", c("event_property_damage","Property Damage ($’000)"), c("event_crop_damage","Crop Damage ($’000)"), sprintf("Top 10 Economic impacts %d - 2011", start_year))
...
```{r resultsstates}
summary_table(analysis_annual_states, "STATE", c("event_fatalities","Fatalities"), c("event_injuries","Injuries"), sprintf("Top 10 Public Health impacts %d - 2011", start_year))


summary_table(analysis_annual_states, "STATE", c("event_property_damage","Property Damage ($’000)"), c("event_crop_damage","Crop Damage ($’000)"), sprintf("Top 10 states with the greated Economic impacts %d - 2011", start_year))
